...
function createTextTexture(
  ...
  const fontSize = 80; // High resolution font rendering
...
class Title {
  ...
  createMesh() {
    ...
    // Make text proportional, smaller on mobile-like ratios
...
class Media {
  ...
  update(
    ...
    // Reduce bend on mobile so cards don't overlap completely
    const currentBend = this.screen.width < 768 ? Math.min(Math.abs(this.bend), 1) * Math.sign(this.bend) : this.bend;
  ...
  onResize(
    ...
    // Base scale varies by screen orientation
    this.scale = isMobile ? this.screen.width / 600 : this.screen.height / 1500;
    
    // Adjust aspect ratios for mobile so they don't get too tall and skinny
    const baseWidth = isMobile ? 400 : 700;
    const baseHeight = isMobile ? 550 : 900;
...
class App {
  ...
  addEventListeners() {
    ...
    this.container.addEventListener("touchstart", this.boundOnTouchDown as EventListener, { passive: false });
    window.addEventListener("touchmove", this.boundOnTouchMove as EventListener, { passive: false });
  ...
  return (
    <div
      ...
      className={cn(
        ...
        "text-stone-800 font-medium text-[24px]", // changed default styling for Luxe project
        fontClassName,
        className,
      )}
      {...props}
